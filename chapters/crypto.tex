...
% se usi ``'' al posto di "" LaTeX te le mette in "pairs". Ti conviene sostituirlo in tutto il documento con qualcosa di Regex perché è oggettivamente utile fare ``'' (premi Alt Gr + apostrofo/punto di domanda; ma sicuro lo sai già)
...
Per quanto riguarda la cifratura autenticata, ASCON utilizza un approccio duplex-sponge\cite{ascon-specification} % secondo me ha senso mettere il \cite{} dove introduci il concetto, non dove lo speghi; quindi immediatamente
,ovvero un tipo di architettura crittografica che combina due concetti:
\begin{enumerate}[label=\Roman*.] % è solo per estetica, poi as you wish
